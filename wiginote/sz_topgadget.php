...
// BEGIN action
if($action == "pridat")	window_open("sz_topgadget_pridat.php", "", "width=1024,height=400");
if($action == "cpost_export")	window_open("apps.php?app=export_cpost&z=$ids_str");
if($action == "intime_export")	window_open("apps.php?app=export_intime&ids_str=$ids_str&shop=topgadget");
if($action == "tisk_faktur")	window_open("faktura_pdf_topgadget.php?id=$ids_str&shop=topgadget");
if($action == "load_file") include "apps/load_file.php";
// END action

echo "
	<form action='sz_topgadget.php' method='POST' ENCTYPE='multipart/form-data'>
...
$mm = 0;
//if(empty($search)) $sql = "SELECT * FROM `$db`.`zasilky_topgadget`";else 
if($dledopravy != 0) $dd = "`zpusob_dodani` = '$dledopravy' and";
else $dd = "";
$sql = "SELECT * FROM `$db`.`zasilky_topgadget` WHERE $dd (
`vs` LIKE '%$search%' or
`nick` LIKE '%$search%' or
`cz` like '%$search%'
)
ORDER BY  `zasilky_topgadget`.`datum_obednavky` DESC
limit 0, $limit";
...
while($d = mysql_fetch_array($q))
{
	...
	{	
		$sql = "UPDATE  `$db`.`zasilky_topgadget` SET  `cz` =  '".$_POST["cz_$id"]."',
	`paid` =  '".$_POST["paid_$id"]."',
	`send` =  '".$_POST["send_$id"]."',
	`note` =  '".$_POST["note_$id"]."',
	`zpusob_platby` =  '".$_POST["platba_$id"]."',
	`zpusob_dodani` =  '".$_POST["doprava_$id"]."' WHERE  `zasilky_topgadget`.`id` = $id";

 	mysql_query($sql, $spojeni);
	}
	// END edit
	
	$z = new zasilky_topgadget($id);
	...
	if($d["avizovano"] == 1) 
	{
		...
		{
			$sql = "UPDATE  `$db`.`zasilky_topgadget` SET  `avizovano` =  '0' WHERE  `zasilky_topgadget`.`id` = $id;";
			mysql_query($sql, $spojeni);
			$avizovano = "";
		}
	}
	else 
	{
		...
		if(isset($_POST["avizace_$id"]))
		{
			$sql = "UPDATE  `$db`.`zasilky_topgadget` SET  `avizovano` =  '1' WHERE  `zasilky_topgadget`.`id` = $id;";
			mysql_query($sql, $spojeni);
			$avizovano = "CHECKED";
		}
	}
	...
	echo "
		...
		<td width='20'><a onClick=\"window.open('sz_topgadget_podrobnosti.php?id=$id&shop=$shop', 'id', 'width=800,height=400,left=0,top=0,location=no,scrollbars=yes')\"><img src='var/img/main/plus.png' border='0'></a></td>
		...
		<td width='100'><a onClick=\"window.open('faktura_pdf_topgadget.php?id=$id&shop=$shop', 'faktura', 'width=800,height=400,left=0,top=0,location=no,scrollbars=yes')\"><img src='var/img/main/pdf.jpg' border='0'></a></td>
