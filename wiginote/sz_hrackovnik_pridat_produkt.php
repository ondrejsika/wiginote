...
if(empty($krok))
{
		...
		echo "
			<form action='sz_hrackovnik_pridat_produkt.php?krok=2&id=$id' method='POST'>
...
else
{
	...
	$sql = "UPDATE  `$db`.`zasilky_hrackovnik` SET  `items` =  '".$d["items"].";$item_id' WHERE  `zasilky_hrackovnik`.`id` = $id;";
