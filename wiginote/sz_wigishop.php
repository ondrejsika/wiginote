...
// BEGIN action
if($action == "pridat")	window_open("sz_wigishop_pridat.php", "", "width=1024,height=400");
if($action == "cpost_export")	window_open("apps.php?app=export_cpost&z=$ids_str");
if($action == "intime_export")	window_open("apps.php?app=export_intime&ids_str=$ids_str&shop=wigishop");
if($action == "tisk_faktur")	window_open("faktura_pdf_wigishop.php?id=$ids_str&shop=wigishop");
if($action == "load_file") include "apps/load_file.php";
// END action

echo "
	<form action='sz_wigishop.php' method='POST' ENCTYPE='multipart/form-data'>
...
$mm = 0;
//if(empty($search)) $sql = "SELECT * FROM `$db`.`zasilky_wigishop`";else 
if($dledopravy != 0) $dd = "`zpusob_dodani` = '$dledopravy' and";
else $dd = "";
$sql = "SELECT * FROM `$db`.`zasilky_wigishop` WHERE $dd (
`vs` LIKE '%$search%' or
`nick` LIKE '%$search%' or
`cz` like '%$search%'
)
ORDER BY  `zasilky_wigishop`.`vs` DESC
limit 0, $limit";
...
while($d = mysql_fetch_array($q))
{
	...
	{	
		$sql = "UPDATE  `$db`.`zasilky_wigishop` SET  `cz` =  '".$_POST["cz_$id"]."',
	`paid` =  '".$_POST["paid_$id"]."',
	`send` =  '".$_POST["send_$id"]."',
	`note` =  '".$_POST["note_$id"]."',
	`zpusob_platby` =  '".$_POST["platba_$id"]."',
	`zpusob_dodani` =  '".$_POST["doprava_$id"]."' WHERE  `zasilky_wigishop`.`id` = $id";

 	mysql_query($sql, $spojeni);
	}
	// END edit
	
	$z = new zasilky_wigishop;
	$z->load($id);

	$pa = new adresy;
	$pa->load($d["adresa_platebni"]);
	...
	$platba = new select("platba_$id",array(0,1,2,3,4),array("","dobírka","na ucet","paypal", "osp"),$z->view("zpusob_platby"));
//select("platba_$id",array(0,1,2),array("","dobirka","naucet"),$z->view("zpusob_platby"));
	...
	if($d["avizovano"] == 1) 
	{
		...
		{
			$sql = "UPDATE  `$db`.`zasilky_wigishop` SET  `avizovano` =  '0' WHERE  `zasilky_wigishop`.`id` = $id;";
			mysql_query($sql, $spojeni);
			$avizovano = "";
		}
	}
	else 
	{
		...
		if(isset($_POST["avizace_$id"]))
		{
			$sql = "UPDATE  `$db`.`zasilky_wigishop` SET  `avizovano` =  '1' WHERE  `zasilky_wigishop`.`id` = $id;";
			mysql_query($sql, $spojeni);
			$avizovano = "CHECKED";
		}
	}
	...
	echo "
		...
		<td width='20'><a onClick=\"window.open('sz_wigishop_podrobnosti.php?id=$id&shop=$shop', 'id', 'width=800,height=400,left=0,top=0,location=no,scrollbars=yes')\"><img src='var/img/main/plus.png' border='0'></a></td>
		...
		<td width='100'><a onClick=\"window.open('faktura_pdf_wigishop.php?id=$id&shop=$shop', 'faktura', 'width=800,height=400,left=0,top=0,location=no,scrollbars=yes')\"><img src='var/img/main/pdf.jpg' border='0'></a></td>
		<td width='100'>".str_replace(" ","&nbsp;",$pa->jmeno)."</td>
