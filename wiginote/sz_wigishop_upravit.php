...
if(empty($action))
{
	...
	echo "
			<form action='sz_wigishop_upravit.php?action=2&id=$id' method='post'>
			<p>vs: ".$d["vs"]."
...
else
{
	...
	$sql = "UPDATE  `$db`.`zasilky_wigishop` SET  `postovne` =  '".$_POST["postovne"]."',
`email` =  '".$_POST["email"]."',
`nick` =  '".$_POST["nick"]."' WHERE  `zasilky_wigishop`.`id` = $id;";
	mysql_query($sql, $spojeni);
