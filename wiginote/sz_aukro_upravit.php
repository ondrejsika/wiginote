...
if(empty($action))
{
	...
	echo "
			<form action='sz_aukro_upravit.php?action=2&id=$id' method='post'>
			<p>vs: ".$d["vs"]."
...
else
{
	...
	$sql = "UPDATE  `$db`.`zasilky_aukro` SET  `postovne` =  '".$_POST["postovne"]."',
`email` =  '".$_POST["email"]."',
`nick` =  '".$_POST["nick"]."' WHERE  `zasilky_aukro`.`id` = $id;";
	mysql_query($sql, $spojeni);

	header("location: sz_aukro_podrobnosti.php?id=$id");
