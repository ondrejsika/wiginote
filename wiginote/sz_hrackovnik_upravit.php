...
if(empty($action))
{
	...
	echo "
			<form action='sz_hrackovnik_upravit.php?action=2&id=$id' method='post'>
			<p>vs: ".$d["vs"]."
...
else
{
	...
	$sql = "UPDATE  `$db`.`zasilky_hrackovnik` SET  `postovne` =  '".$_POST["postovne"]."',
`email` =  '".$_POST["email"]."',
`nick` =  '".$_POST["nick"]."' WHERE  `zasilky_hrackovnik`.`id` = $id;";
	mysql_query($sql, $spojeni);

	header("location: sz_hrackovnik_podrobnosti.php?id=$id");
