...
if(empty($krok))
{
	...
	echo "
			<form action='sz_wigishop_pridat.php?krok=2' method='POST'>
...
elseif($krok == 2)
{
	...
	echo "
			<form action='sz_wigishop_pridat.php?krok=3' method='POST'>
			<p>vs: ".$_POST["vs"]."
			<br>nick: ".$_POST["nick"]."
...
if($krok == 3)
{
	...
	if(!isset($_POST["act"]) or $_POST["act"] == "dalsi")
	{
		...
		echo "
			<form action='sz_wigishop_pridat.php?krok=3&action=2' method='POST'>
			<p>vs: ".$_SESSION["vs"]."
	...
	if($d["zpusob_platby"] == "cashondelivery") $postovne = $dop[1] + 50;
		...
		$sql = "INSERT INTO `$db`.`zasilky_wigishop` (`id`, `shop`, `vs`, `cz`, `paid`, `bv_paid`, `send`, `note`, `doruceno`, `postovne`, `datum_obednavky`, `zpusob_platby`, `zpusob_dodani`, `email`, `nick`, `adresa_platebni`, `adresa_dorucovaci`, `items`, `avizovano`) VALUES ('".$id2."', 'wigishop', '".$d["vs"]."', '".$d["cz"]."', '".$d["paid"]."', '".$d["bv_paid"]."', '".$d["send"]."', '".$d["note"]."', '".$d["doruceno"]."', '$postovne', '".$d["datum_obednavky"]."', '".$_SESSION["zpusob_platby"]."', '".$_SESSION["zpusob_dodani"]."', '".$_SESSION["email"]."', '".$_SESSION["nick"]."', '$adresa_platebni', '$adresa_dorucovaci', '$items', '0');";

		mysql_query($sql, $spojeni);

		$_SESSION["sku"] = $_SESSION["mnozstvi"] = $_SESSION["cena"] = $_SESSION["nazev"] = "";
		header("location: sz_wigishop_podrobnosti.php?id=$id2");
	}
}
/**********/
}
